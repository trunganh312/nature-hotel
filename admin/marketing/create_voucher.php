...
$Auth->checkPermission('mkt_voucher_create');

/** --- Khai báo một số biến cơ bản --- **/
$page_title =   'Thêm mới Voucher';
/** --- End of Khai báo một số biến cơ bản --- **/

/** --- Class query để insert dữ liệu --- **/
$Query  =   new GenerateQuery('voucher');
$Query->add('vou_type', DATA_INTEGER, 0, 'Bạn chưa chọn kiểu giảm giá')
        ->add('vou_value', DATA_INTEGER, 0)
        ->add('vou_time_create', DATA_INTEGER, CURRENT_TIME)
        ->add('vou_last_modify', DATA_INTEGER, CURRENT_TIME)
        ->add('vou_title', DATA_STRING, '')
        ->add('vou_message', DATA_STRING, '')
        ->add('vou_description', DATA_STRING, '')
        ->add('vou_quantity', DATA_INTEGER, 1);
...
if ($Query->submitForm()) {
    ...
    $vou_code   =   getValue('vou_code', GET_STRING, GET_POST, '');
    if ($vou_code == '') {
        $vou_code   =   $VoucherModel->generateRandomCode();
    } else {
        $vou_code   =   strtoupper($vou_code);
        //Kiểm tra xem có bị trùng mã ko
        $check      =   $VoucherModel->getVoucherInfoByCode($vou_code, "vou_id");
        if (!empty($check)) {
            $Query->addError('Mã voucher này đã tồn tại');
        }
    }
    $_POST['vou_code']  =   $vou_code;
    $Query->add('vou_code', DATA_STRING, $vou_code, 'Bạn chưa nhập mã Voucher');
...
<body class="windows-thickbox">
    ...
    <?=$Form->text('Mã Voucher', 'vou_code', isset($vou_code) ? $vou_code : '', false, 'Gồm 7 ký tự chữ. Không bắt buộc.')?>
    ...
    <?=$Form->datePicker('Hạn sử dụng', 'time_expire', getValue('time_expire', GET_STRING, GET_POST, ''), true)?>
    <?=$Form->text('Voucher Title', 'vou_title', $vou_title, false, 'Dùng hiển thị trên website khi giảm giá theo mức tiền')?>
    <?=$Form->text('Voucher Message', 'vou_message', $vou_message, false)?>
