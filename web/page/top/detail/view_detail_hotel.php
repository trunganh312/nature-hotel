                        ...
                        <div id="modern-search-result" class="modern-search-result" data-layout="1">
                            <div class="style-list">
                                ...
                                foreach ($list_hotel as $row) {
                                    ...
                                    $dest_near  =   $PlaceModel->getDestinationByLatLon($row['hot_city'], 0, $row['hot_lat'], $row['hot_lon'], 'des_id, des_name, des_name_short, des_image, des_address_full', 15, 7);
                                    $row['url'] =   $Router->detailHotel($row, param_box_web(103));
                                    $row['full_name']   =   $HotelModel->genHotelFullName($row);
                                    ?>
                                    <div class="top_item">
                                        <div>
                                            <h2 id="top_item_<?=$i?>"><?=$i?>. <?=$row['hot_name']?></h2>
                                            <p>Thông tin giới thiệu:</p>
                                            <ul>
                                                <li><b>Tên đầy đủ:</b> <a target="_blank" href="<?=$row['url']?>"><?=$row['full_name']?></a>.</li>
                                                <li><b>Địa chỉ:</b> <?=$row['hot_address_full']?>.</li>
                                                <li><b>Loại hình:</b> <?=$cfg_hotel_type[$row['hot_type']]?>.</li>
                                                <?
                                                if ($row['hot_star'] > 0) {
                                                    echo    '<li><b>Hạng sao:</b> ' . $row['hot_star'] . ' sao.</li>';
                                                }
                                                if ($row['hot_review_score'] > 0) {
                                                    echo    '<li><b>Điểm đánh giá:</b> ' . $row['hot_review_score'] . '/10.</li>';
                                                }
                                                if (!empty($dest_near)) {
                                                    $arr    =   [];
                                                    foreach ($dest_near as $d) {
                                                        $arr[]  =   (!empty($d['des_name_short']) ? $d['des_name_short'] : $d['des_name']);
                                                    }
                                                    echo    '<li><b>Các điểm vui chơi, tham quan ở gần:</b> ' . implode(', ', $arr) . '.</li>';
                                                }
                                                ?>
                                            </ul>
                                            <p class="text-center"><img data-src="<?=$Router->srcHotel($row['hot_id'], $row['hot_picture'])?>" alt="<?=$row['full_name']?>" class="lazyload" /></p>
                                            <p class="text-center text-italic">Hình ảnh <?=$row['full_name']?> (<a target="_blank" href="<?=$row['url']?>">Xem chi tiết</a>).</p>
                                            <div><?=$row['hot_description']?></div>
                                            <h3 class="text-center">
                                                <a class="btn btn-primary" target="_blank" href="<?=$row['url']?>">Xem giá phòng</a>
                                            </h3>
                                        </div>
                                    </div>
                                    <?
                ...
                if (!empty($list_collection)) {
                    ...
                    <div class="list_collection_bottom list_collection_hot box_relate_border">
                        ...
                        $list_d =   $DB->query("SELECT dis_id, dis_name, dis_name, dis_hot
                                                FROM district
                                                WHERE dis_active = 1 AND dis_hot = 1 AND dis_city = " . $collection_info['col_city'])
                                                ->toArray();
                        foreach ($list_d as $d) {
                            ?>
                            <h4 class="top_list_view_more"><i class="fal fa-hand-point-right"></i><a href="<?=$Router->listCollectionDistrict($d, param_box_web(105))?>">Xem thêm các bộ sưu tập ở <?=$PlaceModel->getDistrictName($d)?></a></h4>
                            <?
                        }  
                        ?>
                        <h4 class="top_list_view_more"><i class="fal fa-hand-point-right"></i><a href="<?=$Router->listCollectionCity($collection_info, param_box_web(105))?>">Xem thêm các bộ sưu tập ở <?=$collection_info['cit_name']?></a></h4>
                    </div>
                    <?
                }
                ?>
