        ...
        try {
            ...
            window.addEventListener('roomSelectionChange', function (e) {
                ...
                if (!selectedRooms || selectedRooms.length === 0) {
                    ...
                    $('#total-price').text(`${new Intl.NumberFormat('vi-VN').format(0)} VND`);
                    return;
                }
                ...
                selectedRooms.forEach(roomType => {
                    totalRoomPrice += roomType.roomPrice * roomType.roomCount;
                });
                ...
                window.currentSelectedRooms = selectedRooms;
            });

            $(document).on('click', '.booking-button', function(e) {
                e.preventDefault();

                // Lấy thông tin phòng đã chọn từ biến global (hoặc từ DOM nếu cần)
                const selectedRooms = window.currentSelectedRooms || [];
                // Lấy ngày checkin/checkout và số đêm
                const dateRange = $('.date-range-input').val().split(' - ');
                ...
                if (!checkIn || !checkOut) {
                    alert('Vui lòng chọn đầy đủ ngày nhận/trả phòng!');
                    return;
                }
                ...
                $.ajax({
                    url: '/ajax/save_booking_session.php',
                    type: 'POST',
                    data: {
                        bookingData: JSON.stringify(bookingData)
                    },
                    success: function(res) {
                        // Sau khi lưu thành công thì chuyển sang trang checkout
